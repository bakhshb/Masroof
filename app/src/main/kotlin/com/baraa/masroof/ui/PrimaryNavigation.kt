...
@Composable fun PrimaryNavigation(initialTab: PrimaryTab = PrimaryTab.HOME) {
    ...
    var showSenderMappings by remember { mutableStateOf(false) }
    Scaffold(bottomBar = {
        NavigationBar {
        ...
        Box(Modifier.fillMaxSize().padding(padding)) {
            when (PrimaryTab.values()[tab]) {
                PrimaryTab.HOME -> HomeScreen(
                    onImportMessages = { tab = PrimaryTab.TRANSACTIONS.ordinal },
                    onShowAllTransactions = { tab = PrimaryTab.TRANSACTIONS.ordinal },
                    onOpenReview = { tab = PrimaryTab.TRANSACTIONS.ordinal },
                )
                PrimaryTab.TRANSACTIONS -> TransactionOperationsScreen()
                PrimaryTab.ACCOUNTS -> AccountListScreen(onClose = {})
                PrimaryTab.MORE -> MoreMenu(
                    onSettings = { showSettings = true },
                    onHistory = { showFinancialHistory = true },
                    onRules = { showAccountLinkRules = true },
                    onSenderMappings = { showSenderMappings = true },
                )
            }
        }
    }
    ...
    if (showSenderMappings) SenderMappingsScreen(onClose = { showSenderMappings = false })
}
