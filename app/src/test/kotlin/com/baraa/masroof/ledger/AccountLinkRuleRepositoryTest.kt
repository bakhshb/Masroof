...
class AccountLinkRuleRepositoryTest {
    private class FakeDao : AccountLinkRuleDao {
        val rules = mutableListOf<AccountLinkRuleEntity>(); var idSeq = 0L
        ...
        override suspend fun insert(rule: AccountLinkRuleEntity): Long { idSeq += 1; rules += rule.copy(id = idSeq); return idSeq }
    ...
    @Test fun checkedConfirmationCreatesRule() = runBlocking {
        val dao = FakeDao(); val repo = AccountLinkRuleRepository(dao); repo.remember(tx(), account(1), "source"); assertEquals(1, dao.rules.size); assertTrue(dao.rules.single().active) }
    @Test fun existingRuleIsUpdatedInsteadOfDuplicated() = runBlocking {
        val dao = FakeDao(); val repo = AccountLinkRuleRepository(dao); val a = tx(); repo.remember(a, account(1), "source"); val before = dao.rules.single().confirmationCount
        repo.remember(a, account(1), "source"); assertEquals(1, dao.rules.size); assertEquals(before + 1, dao.rules.single().confirmationCount) }
    @Test fun findRespectsActiveFlag() = runBlocking {
        val dao = FakeDao(); val repo = AccountLinkRuleRepository(dao); repo.remember(tx(), account(1), "source"); val rule = dao.rules.single(); dao.update(rule.copy(active = false)); assertNull(repo.find(tx(), listOf(account(1)))) }
    @Test fun signatureExcludesBodyAndAmount() {
        ...
        assertFalse(signature.contains("999999")); assertFalse(signature.contains("secret"))
    }
}